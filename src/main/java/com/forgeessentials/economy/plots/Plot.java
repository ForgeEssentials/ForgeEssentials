...
public class Plot {
	...
	/**
	 * Gets the size that counts for price / limit calculation. Depending on whether
	 * the column flag is set or not, this is the area or volume of the plot.
	 * 
	...
	public void setDefaultPermissions() {
		zone.setGroupPermissionProperty(GROUP_ALL, PERM_OWNER,
				owner == null ? SERVER_OWNER : owner.getOrGenerateUuid().toString());
		if (owner != null)
		...
		// zone.setGroupPermission(GROUP_ALL, ModuleProtection.PERM_INTERACT_ENTITY +
		// Zone.ALL_PERMS, false);
	}

	public void printInfo(CommandSource sender) {
		ChatOutputHandler.chatNotification(sender,
				String.format("#%d: \"%s\" at %s", zone.getId(), getName(), getCenter().toString()));
	}
	...
	public void printDetails(CommandSource sender) {
		...
		ChatOutputHandler.chatNotification(sender, String.format("  Location between %s and %s",
				zone.getArea().getHighPoint().toString(), zone.getArea().getLowPoint().toString()));
		long price = getPrice();
		if (price >= 0)
	...
	public static boolean define(WorldArea area, UserIdent owner) throws EventCancelledException {
		...
		if (isColumnMode(area.getDimension()))
			area = new WorldArea(area.getDimension(),
					area.getHighPoint().setY(ServerLifecycleHooks.getCurrentServer().getMaxBuildHeight()),
	...
	static void registerPermissions() {
		...
		perms.registerPermissionProperty(PERM_PRICE, "1",
		...
		perms.registerPermissionDescription(PERM_SELL_PRICE,
				"Price that the plot can be bought for by other players (sell offer)");
		...
		perms.registerPermission(PERM_DELETE, DefaultPermissionLevel.OP,
				"Allows a player to delete any plots, including plots not owned by him.");
