...
public class CommandSellprice extends ForgeEssentialsCommandBuilder
{
    ...
    public LiteralArgumentBuilder<CommandSource> setExecution()
    {
        return baseBuilder
                ...
                .then(Commands.literal("generate")
                        .executes(CommandContext -> execute(CommandContext, "generate")
                                )
                        )
    ...
    public int execute(CommandContext<CommandSource> ctx, String params) throws CommandSyntaxException
    {
        ...
        {
            ChatOutputHandler.chatConfirmation(ctx.getSource(), "/sellprice save                 -Save generated prices to config");
            ChatOutputHandler.chatConfirmation(ctx.getSource(), "/sellprice set <item> <price>  -Set a price for an item");
            ChatOutputHandler.chatConfirmation(ctx.getSource(), "/sellprice generate            -Generate default prices");
    ...
    public static void calcPriceList(CommandContext<CommandSource> ctx, boolean save)
    {
	                        ...
	                        if (ingredient != null)
	                        {
	                            ...
	                            if (stack != ItemStack.EMPTY)
	                            {
	                            	try {
	                            		...
	                            		//This happens if a crafting recipe has a 'blank' spot in it, like a chest with a center 'hole' in the recipe
	                ...
	                do
	                {
	                    ...
	                    for(IRecipe<?> recipe : recpies)
	                    {
	                        ...
	                        if (price > 0)
	                        {
	                            ...
	                            Double resultPrice = priceMap.get(ServerUtil.getItemName(recipe.getResultItem()));
	                            if (resultPrice == null || price < resultPrice)
	                            {
	                                priceMap.put(ServerUtil.getItemName(recipe.getResultItem()), price);
	                                changedPrice = true;
	                                ...
	                                for (Ingredient ingredient : getRecipeItems(recipe))
	                                    if (ingredient != null)
	                                    {
	                                        ...
	                                        if (stack != ItemStack.EMPTY)
	                                            msg += String.format("\n  %.0f - %s", priceMap.get(ServerUtil.getItemName(stack)),
	                                                    ServerUtil.getItemName(stack.getItem()));
	                                    }
	                                writer.write(msg + "\n");
	                            }
        ...
        else
        {
            ...
            ChatOutputHandler.chatConfirmation(ctx.getSource(), "You can also use [/sellprice save] to directly save the calculated prices");
        }
    }
        ...
        for (Ingredient ingredient : stackList)
            if (ingredient != null)
            {
                ...
                for (ItemStack stack : stacks)
                {
                    ...
                    String id = ServerUtil.getItemName(stack);
                    priceMapFull.put(id, 0.0);
