...
public class CommandSellprice extends ForgeEssentialsCommandBuilder {
	...
	public static void calcPriceList(CommandContext<CommandSource> ctx, boolean save) {
		 ...
		 * Item.REGISTRY.getNameForObject(a); String bId =
		 * Item.REGISTRY.getNameForObject(b); return aId.compareTo(bId); } catch
			...
			try (BufferedWriter writer = new BufferedWriter(new FileWriter(priceLogFile))) {
				...
				do {
					...
					do {
						...
						/*
						 * for (Iterator<Entry<ItemStack, ItemStack>> iterator =
						 * furnaceRecipes.entrySet().iterator(); iterator.hasNext();) { Entry<ItemStack,
						 * ItemStack> recipe = iterator.next(); Double inPrice =
						 * priceMap.get(ItemUtil.getItemIdentifier(recipe.getKey())); if (inPrice !=
						 * null) { double outPrice = inPrice * recipe.getKey().getCount() /
						 * recipe.getValue().getCount(); Double resultPrice =
						 * priceMap.get(ItemUtil.getItemIdentifier(recipe.getValue())); if (resultPrice
						 * == null || outPrice < resultPrice) {
						 ...
						 * recipe.getValue()), resultPrice == null ? 0 : resultPrice, (int) outPrice,
						 * ServerUtil.getItemName(recipe.getKey().getItem()))); changedPrice = true; } }
						 * }
						 */
