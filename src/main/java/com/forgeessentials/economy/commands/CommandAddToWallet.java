...
public class CommandAddToWallet extends ForgeEssentialsCommandBase {
    ...
    {
        if (args.length != 2)
            throw new TranslatedCommandException("Improper syntax. Please try this instead: <player> <amounttoadd>");
        EntityPlayer player = UserIdent.getPlayerByMatchOrUsername(sender, args[0]);
        int amountToAdd = Integer.parseInt(args[1]);

        if (player == null)
            throw new TranslatedCommandException("Player %s does not exist, or is not online.");
