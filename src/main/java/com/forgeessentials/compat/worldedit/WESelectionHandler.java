...
public class WESelectionHandler implements ISelectionProvider
{
    ...
    {
        LocalSession session = ForgeWorldEdit.inst.getSession(player);
        session.getRegionSelector(session.getSelectionWorld()).selectPrimary(BlockVector3.at(start.getX(), start.getY(), start.getZ()), null);
    }
    ...
    {
        LocalSession session = ForgeWorldEdit.inst.getSession(player);
        session.getRegionSelector(session.getSelectionWorld()).selectSecondary(BlockVector3.at(end.getX(), end.getY(), end.getZ()), null);
    }
    ...
    public void select(ServerPlayerEntity player, World dimension, AreaBase area)
    {
        ...
        selector.selectPrimary(BlockVector3.at(area.getLowPoint().getX(), area.getLowPoint().getY(), area.getLowPoint().getZ()), null);
        selector.selectSecondary(BlockVector3.at(area.getHighPoint().getX(), area.getHighPoint().getY(), area.getHighPoint().getZ()), null);
