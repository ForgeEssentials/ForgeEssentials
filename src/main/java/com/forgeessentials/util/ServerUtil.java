...
public abstract class ServerUtil {
	...
	/* ------------------------------------------------------------ */
	/*
	 * public static void replaceCommand(Class<CommandMessage> clazz, ICommand
	 * newCommand) { try { CommandHandler commandHandler = (CommandHandler)
	 ...
	 * "commandMap", "a", "field_71562_a"); Set<ICommand> commandSet =
	 * ReflectionHelper.getPrivateValue(CommandHandler.class, commandHandler,
	 * CommandSetChecker.FIELDNAME); for (Iterator<Entry<String, ICommand>> it =
	 * commandMap.entrySet().iterator(); it.hasNext();) { Entry<String, ICommand>
	 * command = it.next(); if
	 * (clazz.isAssignableFrom(command.getValue().getClass())) {
	 * commandSet.remove(command.getValue()); commandSet.add(newCommand);
	 * command.setValue(newCommand); } } } catch (Exception e) {
	 * LoggingHandler.felog.error(String.format("Error replacing command /%s",
	 * clazz.getName())); e.printStackTrace(); } if (newCommand instanceof
	 * ForgeEssentialsCommandBuilder) ((ForgeEssentialsCommandBuilder)
	 * newCommand).register(); }
	 * 
	 * public static void replaceCommand(ICommand oldCommand, ICommand newCommand) {
	 * try { CommandHandler commandHandler = (CommandHandler)
	 * FMLCommonHandler.instance().getMinecraftServerInstance().getCommandManager();
	 * for (Iterator<Entry<String, ICommand>> it = commandMap.entrySet().iterator();
	 * it.hasNext();) { Entry<String, ICommand> command = it.next(); if
	 * (command.getValue() == oldCommand) { commandSet.remove(command.getValue());
	 * commandSet.add(newCommand); command.setValue(newCommand); } } } catch
	 * (Exception e) {
	 * LoggingHandler.felog.error(String.format("Error replacing command /%s",
	 * oldCommand.getName())); e.printStackTrace(); } if (newCommand instanceof
	 * ForgeEssentialsCommandBuilder) ((ForgeEssentialsCommandBuilder)
	 * newCommand).register(); }
	 * 
	 * public static void replaceCommand(String command, ICommand newCommand) {
	 * ICommand oldCommand = (ICommand)
	 * FMLCommonHandler.instance().getMinecraftServerInstance().getCommandManager().
	 * getCommands().get(command); if (oldCommand != null)
	 * replaceCommand(oldCommand, newCommand); else
	 * LoggingHandler.felog.error(String.
	 * format("Could not find command /%s to replace", command)); }
	 */
}
