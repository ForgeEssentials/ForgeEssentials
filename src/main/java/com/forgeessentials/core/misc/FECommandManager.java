...
public class FECommandManager {
	...
	public static void register(FECommandData commandData) {
		...
		PermissionManager.registerCommandPermission(name, commandData.getData().getPermissionNode(),
	...
	public static void deregister(FECommandData commandData) {
		/*
		 * if (ServerLifecycleHooks.getCurrentServer() == null) return; CommandHandler
		 * cmdHandler = (CommandHandler)
		 * FMLCommonHandler.instance().getMinecraftServerInstance().getCommandManager();
		 * Map<String, ICommand> commandMap = cmdHandler.getCommands(); Set<ICommand>
		 * commandSet = cmdHandler.commandSet;
		 * 
		 * String commandName = getName(); List<String> commandAliases = getAliases();
		 * commandSet.remove(this); if (commandName != null)
		 * commandMap.remove(commandName); if (commandAliases != null &&
		 * !commandAliases.isEmpty()) { for (String alias : commandAliases) {
