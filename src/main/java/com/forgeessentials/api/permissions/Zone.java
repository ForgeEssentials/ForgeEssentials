...
public abstract class Zone
{
    ...
    {
        if (ident != null && !FEApi.getFEEventBus().post(new PermissionEvent.User.ModifyPermission(getServerZone(), ident, this, permissionNode, value)))
        {
    ...
    public boolean clearPlayerPermission(UserIdent ident, String permissionNode)
    {
        ...
        {
            PermissionList map = getPlayerPermissions(ident);
            if (map != null && !FEApi.getFEEventBus().post(new PermissionEvent.User.ModifyPermission(getServerZone(), ident, this, permissionNode, null)))
            {
                map.remove(permissionNode);
                return true;
            }
    ...
    {
        if (group != null && !FEApi.getFEEventBus().post(new PermissionEvent.Group.ModifyPermission(getServerZone(), group, this, permissionNode, value)))
        {
    ...
    public boolean clearGroupPermission(String group, String permissionNode)
    {
        if (group != null)
        {
            PermissionList map = getGroupPermissions(group);
            if (map != null
                    && !FEApi.getFEEventBus().post(new PermissionEvent.Group.ModifyPermission(getServerZone(), group, this, permissionNode, null)))
            {
                map.remove(permissionNode);
                return true;
            }
