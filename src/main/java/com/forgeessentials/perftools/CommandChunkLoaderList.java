...
public class CommandChunkLoaderList extends ForgeEssentialsCommandBuilder {
	 ...
	 * 
	 * for (Ticket ticket :
	 * ForgeChunkManager.getPersistentChunksFor(world).values()) { if
	 * (ticket.isPlayerTicket()) { playerTickets.put(ticket.getPlayerName(),
	 * ticket); } else { modTickets.put(ticket.getModId(), ticket); } }
	 * 
	 * if (modTickets.isEmpty() && playerTickets.isEmpty()) { return; }
	 * 
	 * if (!key.equals("*")) { ChatOutputHandler.chatNotification(ctx.getSource(),
	 * TextFormatting.UNDERLINE + "ChunkLoaders for " + key.split(":", 2)[1] + ":");
	 * }
	 * 
	 * ChatOutputHandler.chatNotification(ctx.getSource(), "Dim " +
	 * world.provider.getDimensionType().getName() + ":");
	 * 
	 * if (key.startsWith("p:") || key.equals("*")) { for (String username :
	 * playerTickets.keySet()) { if (key.replace("p:",
	 * "").equalsIgnoreCase(username) || key.equals("*")) { if (key.equals("*")) {
	 * ChatOutputHandler.chatNotification(sender, username); }
	 * 
	 * HashSet<ChunkPos> chunks = new HashSet<ChunkPos>();
	 * 
	 * for (Ticket ticket : playerTickets.get(username)) { for (Object obj :
	 * ticket.getChunkList()) { chunks.add((ChunkPos) obj); } }
	 * 
	 * for (ChunkPos coords : chunks) { ChatOutputHandler.chatNotification(sender,
	 * coords.x + " : " + coords.z); } } } }
	 * 
	 * if (key.startsWith("m:") || key.equals("*")) { for (String modID :
	 * modTickets.keySet()) { if (key.equals("*")) {
	 * ChatOutputHandler.chatNotification(sender, modID); } HashSet<ChunkPos> chunks
	 * = new HashSet<ChunkPos>();
	 * 
	 * for (Ticket ticket : playerTickets.get(modID)) { for (Object obj :
	 * ticket.getChunkList()) { chunks.add((ChunkPos) obj); } }
	 * 
	 * for (ChunkPos coords : chunks) {
	 * ChatOutputHandler.chatNotification(ctx.getSource(), coords.x + " : " +
	 * coords.z); } } } }
	 */
	@Override
	 ...
	 * ICommandSender sender, String[] args, BlockPos pos) { if (args.length == 1) {
	 * ArrayList<String> options = new ArrayList<String>(); for (String s :
	 * FMLCommonHandler.instance().getMinecraftServerInstance().getPlayerList().
	 * getAvailablePlayerDat()) { options.add(s); } for (ModContainer mod :
	 * Loader.instance().getActiveModList()) { options.add(mod.getName()); }
	 * 
