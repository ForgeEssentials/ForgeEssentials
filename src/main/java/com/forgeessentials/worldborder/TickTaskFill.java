...
public class TickTaskFill implements ITickTask
{
	...
	public TickTaskFill(WorldServer worldToFill, ICommandSender sender,
			boolean restart)
	{
		...
		{
			OutputHandler.chatError(server, "Already running a filler for dim "
					+ dimID + "!");
			return;
		}
		...
		source = sender;
		world = worldToFill;
		border = ModuleWorldBorder.borderMap.get(APIRegistry.perms.getServerZone()
				.getWorldZone(world).getName());

		if (border.shapeByte == 0 || border.rad == 0)
		{
			OutputHandler.chatError(sender,
					"You need to set the worldborder first!");
			return;
		}
		...
		if (restart)
		{
			TickTaskFill saved = DataManager.getInstance().load(
					TickTaskFill.class,
					Integer.toString(worldToFill.provider.dimensionId));
			if (saved == null)
				saved = (TickTaskFill) DataStorageManager
						.getReccomendedDriver()
						.loadObject(
								con,
								Integer.toString(worldToFill.provider.dimensionId));
			if (saved != null)
			{
				OutputHandler.chatWarning(source,
						"Found a stopped filler. Will resume that one.");
				X = saved.X;
				Z = saved.Z;
				speed = saved.speed;
				ticks = saved.ticks;
	...
	private String getETA()
	{
		try
		{
			return FunctionHelper
					.parseTime((int) (todo / speed / FunctionHelper.getTPS()));
		} catch (Exception e)
		{
			return "";
		}
	...
	public void tick()
	{
		...
		if (ticks % (20 * 25) == 0)
		{
			OutputHandler.chatNotification(source, "Filler for " + dimID + ": "
					+ getStatus());
		}
		...
		for (int i = 0; i < speed; i++)
		{
			...
			/*
			 * get and populate the chunk manually so that it does not get fully
			 * loaded in, this saves ram and increases performance
			 */
	...
	private boolean next()
	{
		...
		else if (border.shapeByte == 2)
		{
			while (true)
			{
				...
				if (rad >= ModuleWorldBorder.getDistanceRound(centerX, centerZ,
						X, Z))
				{
					break;
				}
	...
	public void onComplete()
	{
		...
		if (!stopped)
		{
			OutputHandler.chatWarning(source, "Filler finished after " + ticks
					+ " ticks.");
			System.out.print("Removed filler? :"
					+ DataManager.getInstance().delete(TickTaskFill.class,
							dimID));
			System.out.print("Removed filler? :"
					+ DataStorageManager.getReccomendedDriver().deleteObject(
							con, dimID));
		}
	...
	public void stop()
	{
		...
		OutputHandler.chatWarning(source, "Filler stopped after " + ticks
				+ " ticks. Still to do: " + todo + " chuncks.");
		System.gc();
	}
