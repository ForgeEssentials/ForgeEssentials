    ...
    public void parse(CommandParserArgs arguments) throws CommandException
    {
        ...
        for (GroupEntry group : APIRegistry.perms.getServerZone().getStoredPlayerGroupEntries(ident))
            if (!Zone.PERMISSION_TRUE.equals(APIRegistry.perms.getServerZone().getGroupPermission(group.getGroup(), FEPermissions.GROUP_PROMOTION)))
            {
                ...
                if (ident.hasPlayer())
                    ChatOutputHandler.chatConfirmation(ident.getPlayer().createCommandSourceStack(), Translator.format("You have been removed from the %s group", group));
            }
        ...
        if (ident.hasPlayer())
            ChatOutputHandler.chatConfirmation(ident.getPlayer().createCommandSourceStack(), Translator.format("You have been added to the %s group", groupName));
    }
