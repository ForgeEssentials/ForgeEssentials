...
{
	public static final File wbconfig = new File(ForgeEssentials.FEDIR,
			"WorldBorder.cfg");
	...
	public static void penaltiesConfig(Configuration config)
	{
		...
		config.addCustomCategoryComment("Penalties",
				"This is what will happen to the player if he passes the world boder.");

		String[] stages = { "Stage1" };
		stages = config.get("Penalties", "stages", stages,
				"If you add an item here, a subcategory will be generated").valueList;
		...
		for (String stage : stages)
		{
			...
			effects = config.get(cat, "effects", effects,
					"Get the list of possibilitys in the example file").valueList;
			...
			for (String effect : effects)
			{
				try
				{
					Class c = Class.forName(penaltyBasePackage
							+ effect.toLowerCase());
					try
					{
					...
					{
						OutputHandler.SOP("Could not initialize '" + effect
								+ "' in stage '" + stage + "'");
					}
				} catch (ClassNotFoundException e)
				{
					OutputHandler.SOP("'" + effect + "' in the stage '" + stage
							+ "' does not exist!");
				}
			}
	...
	public static void commonConfig(Configuration config)
	{
		...
		ModuleWorldBorder.logToConsole = config.get(category, "LogToConsole",
				true, "Enable logging to the server console & the log file")
				.getBoolean(true);
		...
		config.addCustomCategoryComment(category,
				"Location. In all worlds the same!");

		int X = config.get(category, "X", 0).getInt();
		int Z = config.get(category, "Z", 0).getInt();
		int rad = config.get(category, "rad", 0).getInt();
		BorderShape shape = BorderShape.valueOf(config.get(category, "shape",
				"square").value);
		boolean set = config.get(category, "set", false,
				"True if the value is actually set.").getBoolean(false);
	...
	public void forceSave()
	{
		...
		config.get(category, "set", false, "True if the value is actually set.").value = ""
				+ ModuleWorldBorder.set;
		config.get(category, "shape", "square").value = ModuleWorldBorder.shape
				.toString();
	}
