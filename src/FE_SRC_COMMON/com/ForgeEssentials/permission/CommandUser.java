...
public class CommandUser
{
	public static void processCommandPlayer(EntityPlayer sender, String[] args)
	{
		...
		else if(args[1].equalsIgnoreCase("supers")) // super perms management
		{
			...
			{
				if(args[2].equalsIgnoreCase("true") || args[2].equalsIgnoreCase("allow")) // allowing super perm
				{
			...
			else // improper amount of arguments
			{
				OutputHandler.chatError(sender, Localization.get(Localization.ERROR_BADSYNTAX) + "");
			}
		}
		...
		{
			if(args[1].equalsIgnoreCase("prefix") || args[1].equalsIgnoreCase("suffix")) // prefix/suffix changes
			{
				
			}
			else if(args[1].equalsIgnoreCase("true") || args[1].equalsIgnoreCase("allow")) // allowing player perm
			{
