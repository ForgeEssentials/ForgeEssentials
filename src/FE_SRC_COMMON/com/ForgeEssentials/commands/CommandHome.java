...
public class CommandHome extends ForgeEssentialsCommandBase
{
	...
	public void processCommandPlayer(EntityPlayer sender, String[] args)
	{
		if (args.length == 0)
		{
			...
			if (home == null)
			{
				OutputHandler.chatError(sender,
						Localization.get("message.error.nohome")
		...
		} else if (PermissionsAPI.checkPermAllowed(new PermQueryPlayer(sender,
				getCommandPerm() + ".set")))
		{
			if (args.length >= 1
					&& (args[0].equals("here") || args[0].equals("set")))
			{
				WarpPoint p = new WarpPoint(sender);
				PlayerInfo.getPlayerInfo(sender).home = p;
				sender.sendChatToPlayer(Localization.format(
						"command.home.confirm", p.getX(), p.getY(), p.getZ()));
			} else if (args.length >= 3)
			{
				...
				WarpPoint p = new WarpPoint(sender.worldObj.getWorldInfo()
						.getDimension(), x, y, z, sender.cameraPitch,
						sender.cameraYaw);
				PlayerInfo.getPlayerInfo(sender).home = p;
				sender.sendChatToPlayer(Localization.format(
						"command.home.confirm", p.getX(), p.getY(), p.getZ()));
