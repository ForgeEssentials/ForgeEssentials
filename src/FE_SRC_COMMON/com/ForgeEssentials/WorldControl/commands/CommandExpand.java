...
public class CommandExpand extends WorldControlCommandBase
{
	...
	public void processCommandPlayer(EntityPlayer player, String[] args)
	{
		...
		if(result==PermResult.ALLOW||result==PermResult.PARTIAL) {
			if(args.length>0 && args.length<4) {
				...
				if(args.length==1) {
					if(args[0].equals("vert")||args[0].equals("vertical")) {
						Point low = sel.getLowPoint();
						Point high = sel.getHighPoint();
						low.y = 0;
						high.y = 256;
						info.setPoint1(low);
						info.setPoint2(high);
			...
			int highZ2 = (sel.getHighPoint().z+(dir==FunctionHelper.Direction.SOUTH?reverse:dir==FunctionHelper.Direction.NORTH?-reverse:0));
				...
				OutputHandler.chatConfirmation(player, "Expanded by "+amount+"!");
			}
		}else{
	...
	{
		return "/" + getCommandName() + " vert/<amount> [reverse] [direction(NSEWUD)]";
	}
