...
public class EventHandler
{
	...
	public void playerInteractEvent(PlayerInteractEvent e)
	{
		...
		if (e.entityPlayer.getEntityData().getBoolean("lb"))
		{
			...
			try
			{
				int limit = e.entityPlayer.getEntityData().getInteger(
						"lb_limit");
				Date date = new Date();
				...
				st.execute("SELECT * FROM  `blockchange` WHERE  `Dim` = "
						+ e.entityPlayer.dimension + " AND  `X` = " + e.x
						+ " AND  `Y` = " + e.y + " AND  `Z` = " + e.z
				...
				while (res.next())
				{
					e.entityPlayer.sendChatToPlayer(res.getString("player")
							+ " " + res.getString("category") + " block "
							+ res.getString("block") + " at "
