	...
	private byte[] transformItemStack(byte[] bytes, HashMap<String, String> hm)
	{
		...
		while (methods.hasNext())
		{
			...
			if (m.name.equals(hm.get("targetMethodName"))
					&& m.desc.equals("(L" + hm.get("entityPlayerJavaClassName")
							+ ";L" + hm.get("worldJavaClassName")
							+ ";IIIIFFF)Z"))
			{
				...
				toInject.add(new MethodInsnNode(INVOKESTATIC,
						"com/ForgeEssentials/core/CustomEventFactory",
						"onBlockPlace", "(L" + hm.get("itemstackJavaClassName")
								+ ";L" + hm.get("entityPlayerJavaClassName")
								+ ";L" + hm.get("worldJavaClassName")
								+ ";IIIIFFF)Z"));
				...
				m.instructions.insertBefore(m.instructions.get(offset),
						toInject);
				addedPlace = true;
		...
		while (methods.hasNext())
		{
					...
					&& m.desc.equals("(III)Z"))
			{
				...
				for (int index = 0; index < m.instructions.size(); index++)
				{
					...
					{
						FieldInsnNode blocksListNode = (FieldInsnNode) m.instructions
								.get(index);
						if (blocksListNode.owner.equals(hm
								.get("blockJavaClassName"))
								&& blocksListNode.name.equals(hm
										.get("blocksListFieldName")))
						{
							int offset = 1;
							while (m.instructions.get(index + offset)
									.getOpcode() != ASTORE)
							{
								offset++;
							}
							VarInsnNode blockNode = (VarInsnNode) m.instructions
									.get(index + offset);
					...
					{
						MethodInsnNode mdNode = (MethodInsnNode) m.instructions
								.get(index);
						if (mdNode.owner.equals(hm.get("worldJavaClassName"))
								&& mdNode.name.equals(hm
										.get("getBlockMetadataMethodName")))
						{
							int offset = 1;
							while (m.instructions.get(index + offset)
									.getOpcode() != ISTORE)
							...
							VarInsnNode mdFieldNode = (VarInsnNode) m.instructions
									.get(index + offset);
					...
					if (m.instructions.get(index).getOpcode() == IFNULL)
					{
						...
						toInject.add(new VarInsnNode(ALOAD, 0));
						toInject.add(new FieldInsnNode(GETFIELD, hm
								.get("javaClassName"),
								hm.get("worldFieldName"), "L"
										+ hm.get("worldJavaClassName") + ";"));
						...
						toInject.add(new VarInsnNode(ALOAD, 0));
						toInject.add(new FieldInsnNode(GETFIELD, hm
								.get("javaClassName"), hm
								.get("entityPlayerFieldName"), "L"
						...
						toInject.add(new MethodInsnNode(INVOKESTATIC,
								...
								"onBlockHarvested", "(L"
										+ hm.get("worldJavaClassName")
										+ ";IIIL"
										+ hm.get("blockJavaClassName") + ";IL"
										+ hm.get("entityPlayerJavaClassName")
										+ ";)Z"));
						toInject.add(new JumpInsnNode(IFNE, lmm2Node));
						toInject.add(new InsnNode(ICONST_0));
						toInject.add(new InsnNode(IRETURN));
						...
						m.instructions.insertBefore(
								m.instructions.get(index + offset), toInject);
						addedBreak = true;
						msg("[FE coremod] Patching ItemInWorldManager Complete!");
						break;
					}
