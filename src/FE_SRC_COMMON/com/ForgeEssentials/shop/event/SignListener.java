				...
				if (sign.signText[0] != null && sign.signText[0].equals("[Buy]")) {
					...
					if (sign.signText[2] != null) {
						...
						if (!ID.equalsIgnoreCase("")) {
							...
							if(Block.blocksList[ID1] == null||Item.itemsList[ID1] ==null){
								return;
							}
						} else {
							sign.signText[0] = "\u00A74[Buy]";
					...
					if (sign.signText[3] != null) {
						...
						if (!Cost.equalsIgnoreCase("")) {
							...
							Wallet.removeFromWallet(Cost1, event.entityPlayer);
							event.entityPlayer.inventory.addItemStackToInventory(new ItemStack(ID1, Amount1, 0));
