...
import static com.ForgeEssentials.mcfconfig.Property.Type.*;
...
public class Configuration
{
	...
	public Property getBlock(String category, String key, int defaultID)
	{
		...
		} else
		{
			...
			{
				prop.value.set(0, Integer.toString(defaultID));
			...
			} else
			{
				for (int j = configBlocks.length - 1; j > 0; j--)
				{
					...
					{
						prop.value.set(0, Integer.toString(j));
						configBlocks[j] = true;
						return prop;
					}
				}
		...
		} else
		{
			...
			} else
			{
				for (int x = configItems.length - 1; x >= ITEM_SHIFT; x--)
				{
					...
					{
						prop.value.set(0, Integer.toString(x - ITEM_SHIFT));
						configItems[x] = true;
						return prop;
					}
				}
	...
	public Property get(String category, String key, int defaultValue)
	{
		...
		if (!prop.isIntValue())
		{
			prop.value.set(0, Integer.toString(defaultValue));
		}
		return prop;
	}
	...
	public Property get(String category, String key, boolean defaultValue)
	{
		...
		{
			prop.value.set(0, Boolean.toString(defaultValue));
		}
		return prop;
	}
