...
public class MultiworldSaveHandler // implements ISaveHandler
{
	...
	public File getDimensionDirectory() {
		return new File(DimensionManager.getCurrentSaveRootDirectory(),
				"FEMultiworld/" + world.getName());
	}

	@Override
	public IChunkLoader getChunkLoader(WorldProvider provider) {
		return new AnvilChunkLoader(getDimensionDirectory(), FMLCommonHandler
				.instance().getMinecraftServerInstance().getDataFixer());
	}
	...
	public WorldInfo loadWorldInfo() {
		...
		if (file1.exists()) {
			try {
				CompoundNBT nbttagcompound = CompressedStreamTools
						.readCompressed(new FileInputStream(file1));
				CompoundNBT nbttagcompound1 = nbttagcompound
						.getCompound("Data");
	...
	public void saveWorldInfoData(WorldInfo p_75755_1_, CompoundNBT data) {
		...
		// FMLCommonHandler.instance().handleWorldDataSave(parent, p_75755_1_,
		// dataTag);
		...
		try {
			...
			CompressedStreamTools.writeCompressed(dataTag1,
