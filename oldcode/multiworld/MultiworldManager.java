...
public class MultiworldManager extends ServerEventHandler
	...
	public static final String PERM_PROP_MULTIWORLD = FEPermissions.FE_INTERNAL
			+ ".multiworld";
	 ...
	 */
	protected MultiworldEventHandler eventHandler = new MultiworldEventHandler(
			this);
	...
	public void load() {
		...
		Map<String, Multiworld> loadedWorlds = DataManager.getInstance()
				.loadAll(Multiworld.class);
				...
				switch (e.type) {
					case NO_PROVIDER :
						LoggingHandler.felog.error(
								String.format(e.type.error, world.provider));
						break;
					case NO_WORLDTYPE :
						LoggingHandler.felog.error(
								String.format(e.type.error, world.worldType));
						break;
					default :
	...
	protected void registerWorld(Multiworld world) throws
		  MultiworldException { world.providerId =
		  getWorldProviderId(world.provider); world.worldTypeObj =
		  getWorldTypeByName(world.worldType);
		  
		  // Register dimension with last used id if possible if
		  (DimensionManager.isDimensionRegistered(world.dimensionId))
		  ...
		  DimensionManager.getProviderType(world.dimensionId).setLoadSpawn(
		  false); }
	 ...
	 */
		  ...
		  { if (world.worldLoaded) return; try { // Initialize world settings
		  MinecraftServer mcServer =
		  FMLCommonHandler.instance().getMinecraftServerInstance(); ServerWorld
		  overworld = DimensionManager.getWorld(0); if (overworld == null)
		  throw new
		  ...
		  settings.setGeneratorOptions(world.generatorOptions); WorldInfo info
		  = new WorldInfo(settings, world.name); ServerWorld worldServer = new
		  WorldServerMultiworld(mcServer, savehandler, info, world.dimensionId,
		  overworld, mcServer.profiler, world); worldServer.init(); //
		  ...
		  provider .getDimensionType().setLoadSpawn(true);
		  worldServer.addEventListener(new ServerWorldEventHandler(mcServer,
		  worldServer));
		  
		  mcServer.setDifficultyForAllWorlds(mcServer.getWorldData().
		  getDifficulty()); if (!mcServer.isSingleplayer())
		  worldServer.getWorldInfo().setGameType(mcServer.getWorldData().
		  getDifficulty( ));
		  
		  world.updateWorldSettings(); world.worldLoaded = true; world.error =
		  false;
		  ...
		  FMLOutboundHandler. OutboundTarget.ALL); channel.writeOutbound(msg);
		  } catch (Exception e) { world.error = true; throw e; } }

	public int getWorldProviderId(String providerName) throws
		  MultiworldException { switch (providerName.toLowerCase()) { // We use
		  the hardcoded values as some mods just replace the class //
		  (BiomesOPlenty) case PROVIDER_NORMAL: return 0; case PROVIDER_HELL:
		  return -1; case PROVIDER_END: return 1; default: // Otherwise we try
		  to use the provider classname that was supplied Integer providerId =
		  worldProviderClasses.get(providerName); if (providerId == null) throw
		  new MultiworldException(Type.NO_PROVIDER); return providerId; } }
	 ...
	 */
		  ...
		  for (WorldZone zone :
		  APIRegistry.perms.getServerZone().getWorldZones().values()) { String
		  wn = zone.getGroupPermission(Zone.GROUP_DEFAULT,
		  PERM_PROP_MULTIWORLD); if (wn != null && wn.equals(world.getName()))
		  { if (zone.getDimensionID() != world.dimensionId) { WorldZone newZone
		  = APIRegistry.perms.getServerZone().getWorldZone(world.dimensionId);
		  // Swap the permissions of the multiworld with the one // that's
		  currently taking up it's dimID zone.swapPermissions(newZone); }
		  return; } } }
	 ...
	 */
		  ...
		  = worldsToRemove.iterator(); it.hasNext();) { ServerWorld world =
		  it.next(); // Check with DimensionManager, whether the world is still
		  loaded if (DimensionManager.getWorld(world.provider.getDimension())
		  == null) { if
	 ...
	 */
		  ...
		  worldsToDelete.iterator(); it.hasNext();) { ServerWorld world =
		  it.next(); // Check with DimensionManager, whether the world is still
		  loaded if (DimensionManager.getWorld(world.provider.getDimension())
		  == null) { try { if
		  (DimensionManager.isDimensionRegistered(world.provider.getDimension()
	 ...
	 */
		  public void loadWorldProviders() { try { Map<DimensionType,
		  IntSortedSet> loadedProviders = getRegisteredDimensions(); for
		  (DimensionType provider : loadedProviders.keySet()) { // skip the
		  default providers as these are aliased as 'normal', // 'nether' and
		  'end' if (provider.getId() >= -1 && provider.getId() <= 1) continue;
		  ...
		  worldProviderClasses.put(PROVIDER_END, -1); } catch
		  (SecurityException | IllegalArgumentException e) {
		  e.printStackTrace(); }
		  LoggingHandler.felog.debug("[Multiworld] Available world providers:"
		  ); for (Entry<String, Integer> provider :
		  worldProviderClasses.entrySet()) { LoggingHandler.felog.debug("# " +
		  provider.getValue() + ":" + provider.getKey()); } }
	 ...
	 */
	public WorldType getWorldTypeByName(String worldType)
			throws MultiworldException {
