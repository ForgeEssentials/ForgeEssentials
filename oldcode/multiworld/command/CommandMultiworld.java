...
public class CommandMultiworld extends ParserCommandBase
{
		...
		switch (subCmd) {
			...
			default :
				throw new TranslatedCommandException(
						FEPermissions.MSG_UNKNOWN_SUBCOMMAND, subCmd);
		}
	}
     ...
     */
         ...
         BaseCommand.getListOfStringsMatchingLastWord(arguments.peek(), ModuleMultiworld.getMultiworldManager() .getWorldProviders().keySet()); return; }if(!arguments.isEmpty())provider=arguments.remove();
	...
	BaseCommand.getListOfStringsMatchingLastWord(arguments.peek(),ModuleMultiworld.getMultiworldManager().getWorldTypes().keySet());return;}if(!arguments.isEmpty())worldType=arguments.remove();

	String generatorOptions = "";if(!arguments.isEmpty())
	{
	...
	public static void parseList(CommandParserArgs arguments)
		...
		String subArg = arguments.isEmpty()
				? "worlds"
				: arguments.remove().toLowerCase();
		switch (subArg) {
			case "providers" :
			...
			case "worldtypes" :
				...
				for (String worldType : ModuleMultiworld.getMultiworldManager()
			...
			default :
				...
				for (Multiworld world : ModuleMultiworld.getMultiworldManager()
						.getWorlds()) {
					arguments.confirm("#" + world.getDimensionId() + " "
							+ world.getName() + ": " + world.getProvider());
				}
				break;
		}
	}
