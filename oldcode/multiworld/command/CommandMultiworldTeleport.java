...
public class CommandMultiworldTeleport extends ParserCommandBase {
	...
	public void parse(CommandParserArgs arguments) throws CommandException {
		...
		if (worldName.equalsIgnoreCase("list")) {
			arguments.confirm("List of worlds: %s",
					StringUtils.join(worldNames, ", "));
			return;
		}

		ServerPlayerEntity player = arguments.parsePlayer(true, true)
				.getPlayerMP();
		if (player == null)
		...
		if (!arguments.isEmpty()) {
			if (arguments.size() < 3)
				throw new TranslatedCommandException(
						"Too few arguments for location.");
			x = arguments.parseDouble();
			y = arguments.parseDouble();
			z = arguments.parseDouble();
		}

		Multiworld multiworld = ModuleMultiworld.getMultiworldManager()
				.getMultiworld(worldName);
		...
		// TranslatedCommandException("You are not allowed to teleport to that
		// dimension" );
