...
public class ModuleBackup extends ConfigLoaderBase
{
    ...
    public static final String[] DEFAULT_EXCLUDE_PATTERNS = new String[] { "DIM-?\\d+", "FEMultiworld", "FEData_backup",
            "DimensionalDoors", };
    ...
    private void registerBackupTask()
    {
        ...
        if (backupInterval > 0)
            TaskRegistry.scheduleRepeated(backupTask, 1000L * 60 * backupInterval);
    }
    ...
    public void bakeConfig(boolean reload)
    {
        ...
        for (String pair : FEbackupOverrides.get())
        {
            ...
            if (parts[0] == null || parts[1] == null
                    || (!parts[1].toLowerCase().equals("true") && !parts[1].toLowerCase().equals("false")))
            {
                throw new IllegalArgumentException(
                        "Expected diffrent from BackupOverrides field, please check your configs, got the value["
                                + parts[1] + "]:key[" + parts[0] + "]");
            }
    ...
    public void load(ForgeConfigSpec.Builder BUILDER, boolean isReload)
    {
        ...
        FEbackupInterval = BUILDER.comment("Automatic backup interval in minutes (0 to disable)")
                .defineInRange("backup_interval", 60, 0, Integer.MAX_VALUE);
        FEbackupOnLoad = BUILDER.comment("Always backup worlds when loaded (server starts)").define("backup_on_load",
                true);
        FEbackupOnUnload = BUILDER.comment("Always backup when a world is unloaded").define("backup_on_unload", true);
        FEkeepBackups = BUILDER.comment("Keep at least this amount of last backups").defineInRange("keep_backups", 12,
                0, Integer.MAX_VALUE);
        FEdailyBackups = BUILDER.comment("Keep at least one daily backup for this last number of last days")
                .defineInRange("keep_daily_backups", 7, 0, Integer.MAX_VALUE);
        FEweeklyBackups = BUILDER.comment("Keep at least one daily backup for this last number of last days")
                .defineInRange("keep_weekly_backups", 8, 0, Integer.MAX_VALUE);
    ...
    public static void backup(ServerWorld dimension)
    {
        ...
        if (world == null)
        {
            ModuleBackup.notify(Translator.format("Dimension %s does not exist or is not loaded",
    ...
    private static synchronized void backup(ServerWorld world, boolean notify)
    {
        ...
        if (!saveWorld(world))
        {
            notify(String.format("Backup of dim %s failed: Could not save world",
        ...
        if (!backupDir.exists())
            if (!backupDir.mkdirs())
            {
                notify(String.format("Backup of dim %s failed: Could not create backup directory",
                        world.dimension().location().toString()));
                return;
            }
        ...
        catch (Exception ex)
        {
            LoggingHandler.felog.error(
                    String.format("Severe error during backup of dim %s", world.dimension().location().toString()));
            ex.printStackTrace();
            if (notify)
    ...
    private static boolean saveWorld(ServerWorld world)
    {
        ...
        catch (Exception e)
        {
            LoggingHandler.felog
                    .error(String.format("Could not save world %s", world.dimension().location().toString()));
            return false;
        }
        // catch (Exception e)
        // {
    ...
    private static void cleanBackups()
    {
        ...
        for (File backupDir : baseDir.listFiles())
        {
            ...
            Calendar oldestBackup = oldestDailyBackup.before(oldestWeeklyBackup) ? oldestDailyBackup
                    : oldestWeeklyBackup;
            ...
            for (Iterator<Entry<Calendar, File>> it = files.entrySet().iterator(); it.hasNext();)
            {
                ...
                {
                    LoggingHandler.felog.debug("Removing Old Backup {} {}",
                            FILE_FORMAT.format(backup.getKey().getTime()), backup.getValue().getParentFile().getName());
            ...
            while (currentDailyBackup.before(now))
            {
                ...
                for (Iterator<Entry<Calendar, File>> it = files.entrySet().iterator(); it.hasNext();)
                {
                    ...
                    if (backup.getKey().after(oldestDailyBackup) || backup.getKey().after(nextDate))
                        break;
                    LoggingHandler.felog.debug("Removing Daily Backup {} {}",
                            FILE_FORMAT.format(backup.getKey().getTime()), backup.getValue().getParentFile().getName());
                    if (!backup.getValue().delete())
                        LoggingHandler.felog.error(
                                String.format("Could not delete backup file %s", backup.getValue().getAbsolutePath()));
            ...
            while (currentWeeklyBackup.before(now))
            {
                ...
                for (Iterator<Entry<Calendar, File>> it = files.entrySet().iterator(); it.hasNext();)
                {
                    ...
                    if (backup.getKey().after(oldestDailyBackup) || backup.getKey().after(nextDate))
                        break;
                    LoggingHandler.felog.debug("Removing Weekly Backup {} {}",
                            FILE_FORMAT.format(backup.getKey().getTime()), backup.getValue().getParentFile().getName());
                    if (!backup.getValue().delete())
                        LoggingHandler.felog.error(
                                String.format("Could not delete backup file %s", backup.getValue().getAbsolutePath()));
    ...
    private static void notify(String message)
    {
        ...
        ChatOutputHandler.sendMessage(ServerLifecycleHooks.getCurrentServer().createCommandSourceStack(),
                messageComponent);
    }
}
