...
public final class MapperUtil {
	...
	public static void renderChunk(BufferedImage image, int offsetX,
			int offsetY, Chunk chunk) {
		for (int ix = 0; ix < 16; ix++) {
			for (int iz = 0; iz < 16; iz++) {
				int iy = chunk.getMaxBuildHeight();
				for (; iy >= 0; iy--) {
					BlockState blockState = chunk
							.getBlockState(new BlockPos(ix, iy, iz));
					if (blockState.getBlock() == Blocks.AIR)
						continue;
					image.setRGB(offsetX + ix, offsetY + iz,
							getBlockColor(blockState,
									blockState.getMapColor(chunk.getLevel(),
	...
	public static BufferedImage renderRegion(ServerWorld world, int regionX,
			int regionZ) {
		int chunkStartX = regionX * MapperUtil.REGION_CHUNKS;
		int chunkStartZ = regionZ * MapperUtil.REGION_CHUNKS;
		// File regionFile = new File(file2, "r." + regionX + "." + regionZ +
		// ".mca");
		BufferedImage image = new BufferedImage(MapperUtil.REGION_BLOCKS,
				MapperUtil.REGION_BLOCKS, BufferedImage.TYPE_3BYTE_BGR);
		for (int rx = 0; rx < MapperUtil.REGION_CHUNKS; rx++) {
			for (int rz = 0; rz < MapperUtil.REGION_CHUNKS; rz++) {
				...
				MapperUtil.renderChunk(image, rx * MapperUtil.CHUNK_BLOCKS,
						rz * MapperUtil.CHUNK_BLOCKS, chunk);
			}
		}
		return image;
	}
	...
	public static Chunk loadOrCreateChunk(ServerWorld world, int cx, int cz) {
		...
		if (chunk == null) {
			...
			try {
				Method getVisibleChunkIfPresentMethod = ChunkManager.class
						.getDeclaredMethod("getVisibleChunkIfPresent",
								long.class);
				getVisibleChunkIfPresentMethod.setAccessible(true);
				ChunkHolder chunkHolder = (ChunkHolder) getVisibleChunkIfPresentMethod
						.invoke(chunkManager, chunkPos.toLong());
				if (chunkHolder != null) {
					CompletableFuture<Either<IChunk, ChunkHolder.IChunkLoadingError>> future = chunkHolder
							.getFutureIfPresent(
									chunkHolder.getLastAvailableStatus());
					future.thenAccept(either -> {
						if (either.left().isPresent()) {
							// Chunk is now loaded in memory, and will be
							// generated by the server when it has available
							// resources
						}
					});
				}
	...
	public static boolean chunkExists(ServerWorld world, int cx, int cz) {
		...
		try {
			ChunkPos chunkPos = new ChunkPos(cx, cz); // replace x and z with
														// the coordinates of
														// the chunk
			Method method = RegionFileCache.class
					.getDeclaredMethod("getRegionFile", ChunkPos.class);
			method.setAccessible(true);
			RegionFile region = (RegionFile) method.invoke(null, chunkPos);
			doesExist = region
					.doesChunkExist(new ChunkPos(cx & 0x1F, cz & 0x1F));
	...
	public static Color getBlockColor(BlockState blockState,
			MaterialColor matColor) {
		Color color = new Color(matColor.col);
		/*
		 * int id = ; if (id >= colors.length) return Color.BLACK;
		 * 
		 * if (datacolors[id] != null && meta < datacolors[id].length &&
		 * datacolors[id][meta] != null) return datacolors[id][meta][0]; if
		 * (colors[id] != null && colors[id][0] != null) return colors[id][0];
		 * 
		 * // IIcon icon = block.getIcon(ForgeDirection.UP.ordinal(),
		 * chunk.getBlockMetadata(ix, iy, iz)); // if (icon == null) continue;
		 * 
		 * Color color; if (colors[id] != null && colors[id].length > 0 &&
		 * colors[id][0] != null) color = colors[id][0]; else color =
		 * Color.BLACK;
		 * 
		 * if (id >= colors.length) { Color[][] newColors = new Color[id + 1][];
		 * System.arraycopy(colors, 0, newColors, 0, colors.length); colors =
		 * newColors;
		 * 
		 ...
		 * 
		 * if (meta > 0 || datacolors[id] != null) { if (datacolors[id] == null)
		 * datacolors[id] = new Color[16][]; if (datacolors[id][meta] == null)
		 * datacolors[id][meta] = new Color[5]; datacolors[id][meta][0] = color;
		 * datacolors[id][meta][1] = color; datacolors[id][meta][2] = color;
		 * datacolors[id][meta][3] = color; datacolors[id][meta][4] = color; }
		 * else { if (colors[id] == null) colors[id] = new Color[5];
		 * colors[id][0] = color; colors[id][1] = color; colors[id][2] = color;
		 * colors[id][3] = color; colors[id][4] = color; }
		 */
		return color;
	}
	...
	public static void loadColorScheme(InputStream stream) {
		...
		for (int i = 0; i < biomecolors.length; i++) {
			Color[] c = new Color[5];
			int red = 0x80 | (0x40 * ((i >> 0) & 1)) | (0x20 * ((i >> 3) & 1))
					| (0x10 * ((i >> 6) & 1));
			int green = 0x80 | (0x40 * ((i >> 1) & 1)) | (0x20 * ((i >> 4) & 1))
					| (0x10 * ((i >> 7) & 1));
			...
			c[4] = new Color((c[1].getRed() + c[3].getRed()) / 2,
					(c[1].getGreen() + c[3].getGreen()) / 2,
					(c[1].getBlue() + c[3].getBlue()) / 2,
		...
		try {
			...
			while (scanner.hasNextLine()) {
				...
				if ((!isbiome) && (id >= colors.length)) {
					...
					colors = newcolors;
					Color[][][] newdatacolors = new Color[id + 1][][];
					System.arraycopy(datacolors, 0, newdatacolors, 0,
							datacolors.length);
					datacolors = newdatacolors;
				}
				...
				c[0] = new Color(Integer.parseInt(split[1]),
						Integer.parseInt(split[2]), Integer.parseInt(split[3]),
						Integer.parseInt(split[4]));
				c[3] = new Color(Integer.parseInt(split[5]),
						Integer.parseInt(split[6]), Integer.parseInt(split[7]),
						Integer.parseInt(split[8]));
				c[1] = new Color(Integer.parseInt(split[9]),
						Integer.parseInt(split[10]),
						Integer.parseInt(split[11]),
						Integer.parseInt(split[12]));
				c[2] = new Color(Integer.parseInt(split[13]),
			...
			// Log.severe("Could not load colors '" + name + "' ('" +
			// colorSchemeFile + "').", e);
		}
	}
		...
		for (int k = 0; k < c.length; k++) {
			if (c[k] == null) { /* Missing? */
				...
				/* Skip - will backfill when we find next color */
									 ...
									 * No previous color, just backfill this
									 * color
									 */
				...
				for (int kk = idx + 1; kk < k; kk++) {
					...
					for (int jj = 0; jj < cnt; jj++) {
						cc[jj] = new Color(
								(int) ((1.0 - interp) * c[idx][jj].getRed()
										+ interp * c[k][jj].getRed()),
								(int) ((1.0 - interp) * c[idx][jj].getGreen()
										+ interp * c[k][jj].getGreen()),
								(int) ((1.0 - interp) * c[idx][jj].getBlue()
										+ interp * c[k][jj].getBlue()),
								(int) ((1.0 - interp) * c[idx][jj].getAlpha()
