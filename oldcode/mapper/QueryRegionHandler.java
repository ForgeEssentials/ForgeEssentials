...
public class QueryRegionHandler extends GenericRemoteHandler<Request> {
	...
	public QueryRegionHandler() {
		...
		APIRegistry.perms.registerPermission(PERM, DefaultPermissionLevel.ALL,
				"Allows querying region tiles");
	}

	@Override
	public synchronized RemoteResponse<?> handleData(RemoteSession session,
			RemoteRequest<Request> request) {
		if (request.data == null)
			error("missing data");
		ServerWorld world = APIRegistry.namedWorldHandler
				.getWorld(request.data.dim);
		if (world == null)
			error("Invalid dimension");
		try {
			File file = ModuleMapper.getInstance()
					.getRegionFileAsync(world, request.data.x, request.data.z)
					.get();
			if (!file.exists())
			...
			return new RemoteResponse<String>(request,
